...
public class ExpandablePanel extends LinearLayout {
	...
	public void CloseView(int duration) {
		if (mExpanded) {
	...
	public boolean MoveOn(int textLines, int lineSize) {
		Animation a;
		if (mExpanded) {
			if (textLines != 0) {
				a = new ExpandAnimation(mCollapsedHeight
						+ ((textLines - 1) * lineSize), mCollapsedHeight);
			} else {
			...
			if (textLines != 0) {
				a = new ExpandAnimation(mCollapsedHeight, mCollapsedHeight
						+ ((textLines - 1) * lineSize));
			} else {
