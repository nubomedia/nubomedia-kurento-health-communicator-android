...
public class PullToRefreshAttacher implements View.OnTouchListener {
	...
	protected PullToRefreshAttacher(Activity activity, Options options) {
		if (options == null) {
			log.info("Given null options so using default options.");
			options = new Options();
		}
	...
	void addRefreshableView(View view, ViewDelegate viewDelegate,
		...
		if (view == null) {
			log.info("Refreshable View is null.");
			return;
		}
	...
	final boolean onInterceptTouchEvent(View view, MotionEvent event) {
		...
		if (DEBUG) {
			log.debug("onInterceptTouchEvent. Got ViewParams. "
					+ view.toString());
		}
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (canRefresh(true, params.onRefreshListener)
					&& params.viewDelegate.isReadyForPull(view, event.getX(),
							event.getY())) {
		...
		if (DEBUG) {
			log.debug("onInterceptTouchEvent. Returning " + mIsBeingDragged);
		}
	...
	final boolean onTouchEvent(View view, MotionEvent event) {
		...
		final ViewParams params = mRefreshableViews.get(view);
		if (params == null) {
			log.info("View does not have ViewParams");
			return false;
		}
	...
	private void setRefreshingInt(View view, boolean refreshing,
			boolean fromTouch) {
		if (DEBUG) {
			log.debug("setRefreshingInt: " + refreshing);
		}
	...
	/**
	 * HeaderTransformers are what controls and update the Header View to
	 * reflect the current state of the pull-to-refresh interaction. They are
	 * responsible for showing and hiding the header view, as well as update the
	 * state.
	 */
	public static abstract class HeaderTransformer {
		 ...
		 *             {@link #onViewCreated(Activity, View)}
		 *             instead.
		 */
		 ...
		 *            value between 0.0f and 1.0f depending on how far the user
		 *            has pulled.
		 */
	...
	/**
	 * This is used to provide platform and environment specific functionality
	 * for the Attacher.
	 */
	...
	/**
	 * Allows you to specify a number of configuration options when
	 * instantiating a {@link PullToRefreshAttacher}. Used with
	 * {@link #get(Activity, Options) get()}.
	 */
	...
	final static class DecorChildLayout extends FrameLayout {
		...
		protected boolean fitSystemWindows(Rect insets) {
			if (DEBUG) {
				log.debug("fitSystemWindows: " + insets.toString());
			}
